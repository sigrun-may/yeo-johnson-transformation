...
def _transform_yeo_johnson_c() -> Tuple[np.ndarray, np.ndarray]:
    # transform and standardize test and train data
    transformed_train_np, lambdas, skews, error_codes = c_accesspoint.yeo_johnson_power_transformation(
    ...
    np.testing.assert_almost_equal(
        skew(transformed_test_np_stats, axis=0), skew(test, axis=0), decimal=7
    )
