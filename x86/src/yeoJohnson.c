 ...
 */
static int yjFormular1(float y, float lambda, float *result) {
  if (!bBf_set) {
 ...
 * @return int error return code
 */
static int yjFormular2(float y, float *result) {
  if (y == g_max_high_float) {
    printf("\tyjFormular2: overflow y is equal to g_max_low_float\n");
    return -1;
  }
  *result = log(y + 1);
  return 0;
 ...
 * @return int error return code
 */
static int yjFormular3(float y, float lambda, float *result) {
  if (!bBf_set) {
 ...
 * @return int error return code
 */
static int yjFormular4(float y, float *result) {
...
void buildBoundaryBox(float lower_lambda, float upper_lambda) {
  ...
  bBf_yj1.upper_limit =
      pow(upper_lambda * g_max_high_float + 1, 1 / upper_lambda) - 1;
  bBf_yj1.lower_limit =
      pow(lower_lambda * g_max_low_float + 1, 1 / lower_lambda) - 1;

  upper_lambda = (upper_lambda == 2) ? 3 : upper_lambda;
  lower_lambda = (lower_lambda == 2) ? 1 : lower_lambda;
  bBf_yj3.upper_limit = -(
      pow(-g_max_high_float * (2 - upper_lambda) + 1, 1 / (2 - upper_lambda)) -
      1);
  bBf_yj3.lower_limit =
      -(pow(-g_max_low_float * (2 - lower_lambda) + 1, 1 / (2 - lower_lambda)) -
        1);
  bBf_set = 1;
}
