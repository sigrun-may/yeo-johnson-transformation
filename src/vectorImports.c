...
/**
 * @brief Get the Matrix Size From Csv object
 *
...
int importVectorTableFromCsv(char *file_path, matrix *vector_list) {
  ...
  if (errnum != 0) {
    printf("Error: while getting dimensions.\n");
    return -4;
  }
  {
    ...
    if (column_addresses == NULL) {
      printf("Error: failed to allocate memory for column_addresses.\n");
      return -5;
    }
    for (int i = 0; i < vector_list->cols; i++) {
      double *row_info = (double *)malloc(sizeof(double) * vector_list->rows);
      if (row_info == NULL) {
        printf("Error: failed to allocate memory for row_info.\n");
        return -6;
      }
    ...
    if (new_mem == NULL) {
      printf("Error: failed to allocate memory for lambda_storage.\n");
      return -7;
    }
    vector_list->lambda = new_mem;
    new_mem = (double *)malloc(sizeof(double) * vector_list->cols);
    if (new_mem == NULL) {
      printf("Error: failed to allocate memory for skew_storage.\n");
      return -8;
    }
...
void test_getMatrixSizeFromCsv(void) {
  ...
  FILE *wrong_file = fopen(
      "C:\\Users\\jerom\\Documents\\vscode\\Workspace_Yeo\\data\\wrong.csv",
      "r");
  assert_int_equals(getMatrixSizeFromCsv(NULL, column_size, row_size), -1,
                    "Error: file is null but still opened");
  assert_int_equals(getMatrixSizeFromCsv(wrong_file, column_size, row_size), -2,
                    "Error: wrong format but was read");
  fclose(wrong_file);
  FILE *file = fopen(
      "C:\\Users\\jerom\\Documents\\vscode\\Workspace_Yeo\\data\\Mappe2.CSV",
      "r");
  fpos_t file_pointer;
  ...
  assert_int_equals(row_size, 2001,
  ...
  fclose(file);
  file = fopen(
      "C:\\Users\\jerom\\Documents\\vscode\\Workspace_Yeo\\data\\data1.CSV",
      "r");
...
void test_fillMatrixFromCsv(void) {
  ...
  assert_int_euqals(fillMatrixFromCsv(file, NULL, 1, 1), -3,
...
void test_fillMatrixFromCsvf(void) {
  ...
  assert_int_equals(fillMatrixFromCsvf(NULL, test.data, 1, 1), -2,
                    "Error: File is null, should be aborted");
  assert_int_euqals(fillMatrixFromCsvf(file, NULL, 1, 1), -3,
                    "Error: matrix is null, should be aborted");
...
void test_importVectorTableFromCsv(void) {
  char *file_path =
      "C:\\Users\\jerom\\Documents\\vscode\\Workspace_Yeo\\data\\data2.CSV";
  ...
  for (int i = 0; i < vector_list.cols; i++) {
    assert_not_null((void *)*(vector_list.data + i),
                    ("Error: no memory allocated for data[%i]", i));
  }
  ...
  char *wrong_file_path2 =
      "C:\\Users\\jerom\\Documents\\vscode\\Workspace_Yeo\\data\\wrong2.csv";
