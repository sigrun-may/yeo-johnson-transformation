...
static const double max_high_double = DBL_MAX;
static const double max_low_double = -DBL_MAX;
static const double max_high_float = FLT_MAX;
static const double max_low_float = -FLT_MAX;
...
static int bB_set = 0;
static int bBf_set = 0;

static boundaryBox bB_yj1;
static boundaryBox bB_yj3;
static boundaryBoxf bBf_yj1;
static boundaryBoxf bBf_yj3;
...
/**
 * @brief (float) first formular of the Yeo Johnson transformation (y>=0, lambda
 * != 0)
 *
 * @param y value to be transformed
 * @param lambda transformation parameter
...
static int yjFormular1U(double y, double lambda, double *result) {
  *result = (pow(y + 1, lambda) - 1) / lambda;
  return 0;
}

static int yjFormular1Uf(float y, float lambda, float *result) {
  *result = (pow(y + 1, lambda) - 1) / lambda;
  return 0;
}
...
static int yjFormular2(double y, double *result) {
  if (y == max_high_double) {
    printf("yjFormular2: overflow y is equal to max_low_double\n");
    return -1;
  }
  *result = log(y + 1);
  return 0;
...
/**
 * @brief (float) second formular of the Yeo Johnson transformation (y>=0,
 * lambda == 0)
 *
 * @param y value to be transformed
 * @param lambda transformation parameter
...
static int yjFormular2f(float y, float *result) {
  if (y == max_high_float) {
    printf("yjFormular2f: overflow y is equal to max_low_float\n");
    return -1;
  }
  *result = logf(y + 1);
  return 0;
...
static int yjFormular2U(double y, double *result) {
  *result = log(y + 1);
  return 0;
}

static int yjFormular2Uf(float y, float *result) {
  *result = logf(y + 1);
  return 0;
}
...
/**
 * @brief (float) third formular of the Yeo Johnson transformation (y<0, lambda
 * != 2)
 *
 * @param y value to be transformed
 * @param lambda transformation parameter
...
static int yjFormular3U(double y, double lambda, double *result) {
  *result = -(pow(-y + 1, 2 - lambda) - 1) / (2 - lambda);
  return 0;
}

static int yjFormular3Uf(float y, float lambda, float *result) {
  *result = -(powf(-y + 1, 2 - lambda) - 1) / (2 - lambda);
  return 0;
}

/**
 * @brief (double) fourth formular of the Yeo Johnson transformation (y<0,
 * lambda == 2)
 *
 * @param y value to be transformed
 * @param lambda transformation parameter
...
static int yjFormular4(double y, double *result) {
  if (y == max_low_double) {
    printf("yjFormular4: overflow y is equal to max_low_double\n");
    return -1;
  }
  *result = -log(-y + 1);
  return 0;
...
/**
 * @brief (float) fourth formular of the Yeo Johnson transformation (y<0, lambda
 * == 2)
 *
 * @param y value to be transformed
 * @param lambda transformation parameter
...
static int yjFormular4f(float y, float *result) {
  if (y == max_low_float) {
    printf("yjFormular4f: overflow y is equal to max_low_float\n");
    return -1;
  }
  *result = -logf(-y + 1);
  return 0;
...
static int yjFormular4U(double y, double *result) {
  *result = -log(-y + 1);
  return 0;
}

static int yjFormular4Uf(float y, float *result) {
  *result = -logf(-y + 1);
  return 0;
}
...
void buildBoundaryBox(double lower_lambda, double upper_lambda) {
  ...
  bB_yj1.upper_limit =
      pow(upper_lambda * max_high_double + 1, 1 / upper_lambda) - 1;
  bB_yj1.lower_limit =
      pow(lower_lambda * max_low_double + 1, 1 / lower_lambda) - 1;

  upper_lambda = (upper_lambda == 2) ? 3 : upper_lambda;
  lower_lambda = (lower_lambda == 2) ? 1 : lower_lambda;
  bB_yj3.upper_limit =
      -(pow(-max_high_double * (2 - upper_lambda) + 1, 1 / (2 - upper_lambda)) -
        1);
  bB_yj3.lower_limit =
      -(pow(-max_low_double * (2 - lower_lambda) + 1, 1 / (2 - lower_lambda)) -
        1);
  bB_set = 1;
}

void buildBoundaryBoxf(float lower_lambda, float upper_lambda) {
  upper_lambda = (upper_lambda == 0) ? 1 : upper_lambda;
  lower_lambda = (lower_lambda == 0) ? -1 : lower_lambda;
  bBf_yj1.upper_limit =
      powf(upper_lambda * max_high_float + 1, 1 / upper_lambda) - 1;
  bBf_yj1.lower_limit =
      powf(lower_lambda * max_low_float + 1, 1 / lower_lambda) - 1;

  upper_lambda = (upper_lambda == 2) ? 3 : upper_lambda;
  lower_lambda = (lower_lambda == 2) ? 1 : lower_lambda;
  bBf_yj3.upper_limit =
      -(powf(-max_high_float * (2 - upper_lambda) + 1, 1 / (2 - upper_lambda)) -
        1);
  bBf_yj3.lower_limit =
      -(powf(-max_low_float * (2 - lower_lambda) + 1, 1 / (2 - lower_lambda)) -
        1);
  bBf_set = 1;
}
...
/**
 * @brief (float) Yeo Johnson transformation decision tree and error handling
 *
 * @param y value to be transformed
 * @param lambda transformation parameter
 * @param result resulting value after transformation
...
int yjCalculationf(float y, float lambda, float *result) {
  ...
  if (y >= 0) {
    if (lambda != 0) {
      errnum = yjFormular1f(y, lambda, result);
      if (errnum != 0) {
        printf("\texception in yjFormular1f\n");
        return -1;
      }
    } else if (lambda == 0) {
      errnum = yjFormular2f(y, result);
      if (errnum != 0) {
        printf("\texception in yjFormular2f\n");
        return -1;
      }
    }
  } else if (y < 0) {
    if (lambda != 2) {
      errnum = yjFormular3f(y, lambda, result);
      if (errnum != 0) {
        printf("\texception in yjFormular3f\n");
        return -1;
      }
    } else if (lambda == 2) {
      errnum = yjFormular4f(y, result);
      if (errnum != 0) {
        printf("\texception in yjFormular4f\n");
        return -1;
      }
    }
  }
...
int yjCalculationU(double y, double lambda, double *result) {
  if (y >= 0) {
    if (lambda != 0) {
      yjFormular1U(y, lambda, result);
    } else if (lambda == 0) {
      yjFormular2U(y, result);
    }
  } else if (y < 0) {
    if (lambda != 2) {
      yjFormular3U(y, lambda, result);
    } else if (lambda == 2) {
      yjFormular4U(y, result);
    }
  }
  return 0;
}
...
int yjCalculationUf(float y, float lambda, float *result) {
  if (y >= 0) {
    if (lambda != 0) {
      yjFormular1Uf(y, lambda, result);
    } else if (lambda == 0) {
      yjFormular2Uf(y, result);
    }
  } else if (y < 0) {
    if (lambda != 2) {
      yjFormular3Uf(y, lambda, result);
    } else if (lambda == 2) {
...
void test_yj2(void) {
  ...
  assert_int_equals(yjFormular2(max_high_double, &result), -1,
...
void test_yj2f(void) {
  ...
  assert_int_equals(yjFormular2f(max_high_float, &result), -1,
                    "Error: y should overflow but still executed");
  // test success
...
void test_yj4(void) {
  ...
  assert_int_equals(yjFormular4(max_low_double, &result), -1,
                    "Error: y should overflow but still executed");
  // test success
...
void test_yj4f(void) {
  ...
  assert_int_equals(yjFormular4f(max_low_float, &result), -1,
                    "Error: y should overflow but still executed");
  // test success
...
void test_yjCalculation(void) {
  ...
  assert_int_equals(yjCalculation(max_high_double, 0, &result), -1,
  ...
  assert_int_equals(yjCalculation(max_low_double, 2, &result), -1,
...
void test_yjCalculationf(void) {
  ...
  assert_int_equals(yjCalculationf(max_high_float, 0, &result), -1,
  ...
  assert_int_equals(yjCalculationf(max_low_float, 2, &result), -1,
